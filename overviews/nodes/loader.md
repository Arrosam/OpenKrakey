...
- 2026-06-14: code-review fixes — `pushLogEntry` is now re-entrancy-guarded (a closure `emittingLog` flag) so a `log.entry` subscriber that calls `ctx.log.*` can't recurse into a stack overflow; and `load()` failures (plugin import / dependency errors) are now mirrored onto the bus as `core:loader` errors so the inspector's Logs feed surfaces load-time failures.
