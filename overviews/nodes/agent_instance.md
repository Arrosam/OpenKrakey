...
- 2026-06-15: code-review fix — that re-entrancy guard is now PER-AGENT, not module-level. A `{ active }` guard object is created once per `createAgentInstance` and passed to `busLogger(base, bus, source, guard)`; the closure sets/clears `guard.active` instead of a shared module flag. So one agent's synchronous `log.entry` fan-out can no longer suppress another agent's mirror (the module-level flag violated R6). Intra-agent recursion prevention is unchanged; this matches the loader's already per-instance `emittingLog` guard.
