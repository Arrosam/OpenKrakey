 ...
 *
 * The default export IS the Plugin object literal `{ manifest, setup, teardown }`.
 * The loader dynamic-imports one module instance per Agent, so module scope is the
 * per-Agent state: the tool Map, the unsubscribe handles, and the captured context.
 */
...
// --- per-Agent state (module scope = one instance per Agent) ----------------
