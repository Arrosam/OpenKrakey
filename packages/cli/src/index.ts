/**
 * cli — pure, fs-only core (NO `@inquirer/prompts` here).
 *
 * This is the testability seam: every config-file operation lives here as a plain
 * async method over `node:fs/promises`. The interactive shell (src/pages.ts +
 * src/bin.ts) sits on top and is the only place that touches inquirer.
 *
 * Paths in `deps` are already absolute — bin.ts resolves them from PATHS. This
 * module resolves nothing; it only joins agent ids onto `agentsDir` via the
 * shared `agentPaths` helper.
 */
