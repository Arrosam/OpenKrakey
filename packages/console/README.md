...
```
✦ Krakey Console: http://127.0.0.1:7710/
```

The surfaces must be running **separately** — the console only embeds them:

- **Config** — `npm run config:web` (defaults to `http://127.0.0.1:7700`).
...
`http://127.0.0.1:7700` loads the Config shell but its API won't authenticate.
To use the embedded Config **fully**, set `CONFIG_WEB_URL` to include the token
that `config:web` printed, e.g.:

```bash
CONFIG_WEB_URL="http://127.0.0.1:7700/?token=<token>" npm run console
```
