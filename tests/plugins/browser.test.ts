...
const CONFIG_DEFAULTS: Record<string, unknown> = {
  ...
  // F2: persistent-failure ledger bound (default 8; 0 disables the whole feature).
  maxFailureNotices: 8,
};
...
for (const wrong of [
  ...
  { field: "maxFailureNotices", value: "8", expect: 8 },
]) {
...
// battery below). This ring is ALL-OK (no ok:false result recorded), so there is
// no failure ledger to survive and the rendered block goes fully empty.
test("llm.return: clears an ALL-OK results ring (rendered block becomes []), then new results survive", async () => {
  const { store, sys } = await withRing(5);
  emitContextFull(sys, 3); // frame A: shed 3 oldest -> [3,4]
  assert.deepEqual(await survivingTags(store), [3, 4], "frame A shed 3");

  emitLlmReturn(sys); // frame boundary: llm.return DROPS the whole (all-ok) ring

  assert.deepEqual(
    await renderMsgs(resultsBlock(store)),
    [],
    "llm.return empties an all-ok results ring (one-shot delivery; no failures to persist)",
  );
...
// F2 split: these rings carry ONLY ok:true results, so llm.return empties them
// fully (no persistent-failure ledger to survive). The failure-persistence path
// is covered by the F2 battery below.

test("llm.return: >=2 own ok:true tool.results render, then llm.return empties the ring", async () => {
  const { store, sys } = await setup({});
...
// ===========================================================================
// 17. F2 — persistent FAILURE LEDGER, folded into the browser.results block.
//
// NEW behavior (black-box, derived ONLY from the F2 spec — impl does not exist,
// so these are RED against main by design):
//
//   * On an OWN tool.result with ok===false, the plugin upserts a ledger entry
//     keyed by (toolName + NORMALIZED error): trimmed, 'unknown' when empty,
//     capped ~300 chars — carrying {count, firstAt, lastAt}. A repeat of the SAME
//     (tool,error) bumps count; a DISTINCT error is a separate entry.
//   * On an OWN ok===true result for a tool, ALL ledger entries for THAT toolName
//     are removed.
//   * The ledger is bounded by config `maxFailureNotices` (default 8; 0 disables
//     the whole feature — no ledger, no persistent line ever).
//   * Events.LLM_RETURN clears the RESULTS ring + resets pressure but does NOT
//     clear the failure ledger (that is the success/failure split).
//   * Events.CONTEXT_FULL sheds failure entries oldest-first (same slice(round)).
//   * teardown resets the ledger.
//
//   RENDERING (folded into the SAME browser.results MESSAGES block): a ledger
//   entry with count>=2 renders exactly ONE {role:'user', name:'browser'} message
//   naming the tool, the count ('has failed <N>x in a row'), the error, and the
//   reflect-and-stop nudge ('retrying the same call unchanged will NOT succeed').
//   A count-1 failure renders NO persistent line (it only shows as its normal
//   fresh result that frame).
//
// We reuse the existing harness verbatim (setup/emitToolResult/emitLlmReturn/
// emitContextFull/resultsBlock/renderMsgs). No Chrome launch.
// ===========================================================================

// A "persistent failure line" is a rendered message that carries the F2 count
// phrasing 'failed <N>x in a row' (the normal fresh-result entries never do).
...
// The count asserted by a persistent line, or null if it is not a failure line.
...
test("F2 (a): two identical ok:false for one tool -> after llm.return, a count-2 persistent line naming the tool+error renders; the fresh ring entries are gone", async () => {
  const { store, sys } = await setup({});
  emitToolResult(sys, { name: NAVIGATE, ok: false, error: "navigation failed" });
  emitToolResult(sys, { name: NAVIGATE, ok: false, error: "navigation failed" });
  ...
  const line = fails[0];
  assert.match(line, new RegExp(NAVIGATE.replace(".", "\\.")), "persistent line names the tool");
  assert.equal(failureCount(line), 2, "count is 2 (two identical failures in a row)");
  assert.match(line, /navigation failed/, "persistent line carries the error text");
  assert.match(
    line,
    /retrying the same call unchanged will NOT succeed/i,
    "persistent line carries the reflect-and-stop nudge",
  );
  // The one-shot fresh ring entries were cleared by the return; only the
  // persistent ledger line remains.
...
test("F2 (b): two DISTINCT errors from the same tool -> two persistent lines survive llm.return", async () => {
  const { store, sys } = await setup({});
  // Each distinct error must reach count>=2 to render a persistent line.
  emitToolResult(sys, { name: READ_PAGE, ok: false, error: "timeout" });
  emitToolResult(sys, { name: READ_PAGE, ok: false, error: "timeout" });
  emitToolResult(sys, { name: READ_PAGE, ok: false, error: "DOM detached" });
  emitToolResult(sys, { name: READ_PAGE, ok: false, error: "DOM detached" });
  ...
  assert.match(joined, /DOM detached/, "second distinct error present");
  for (const line of fails) {
    assert.equal(failureCount(line), 2, "each distinct error reached count 2");
  }
});
...
test("F2 (c): a subsequent ok:true for the failing tool clears ALL its persistent lines next render", async () => {
  const { store, sys } = await setup({});
  emitToolResult(sys, { name: NAVIGATE, ok: false, error: "boom" });
  emitToolResult(sys, { name: NAVIGATE, ok: false, error: "boom" });
  emitLlmReturn(sys);
  assert.equal(
  ...
  // A success for the SAME tool removes every ledger entry for that toolName.
  emitToolResult(sys, { name: NAVIGATE, ok: true, data: navData("https://ok") });
  emitLlmReturn(sys); // clear the fresh success from the ring too
...
test("F2 (c'): an ok:true for a DIFFERENT tool leaves the first tool's persistent line intact", async () => {
  ...
  emitToolResult(sys, { name: NAVIGATE, ok: false, error: "boom" });
  emitToolResult(sys, { name: NAVIGATE, ok: false, error: "boom" });
  // Success for a different tool must NOT touch NAVIGATE's ledger.
  emitToolResult(sys, { name: LIST_TABS, ok: true, data: { launched: true, tabs: [] } });
  emitLlmReturn(sys);

  const fails = failureLines(await renderMsgs(resultsBlock(store)));
  assert.equal(fails.length, 1, "a different tool's success leaves the ledger entry standing");
  assert.match(fails[0], new RegExp(NAVIGATE.replace(".", "\\.")), "the surviving line is NAVIGATE's");
});
...
test("F2 (d): maxFailureNotices:0 disables the ledger — no persistent line EVER renders", async () => {
  ...
  emitLlmReturn(sys); // would leave a count>=2 line if the feature were on
  assert.deepEqual(
    failureLines(await renderMsgs(resultsBlock(store))),
    [],
...
test("F2 (e): context.full{round:N} sheds failure entries oldest-first, clamped, never throws", async () => {
  ...
  // Three DISTINCT (tool,error) failures, each promoted to count 2 so they render.
  // Recorded oldest-first: errA, errB, errC.
  for (const err of ["errA", "errB", "errC"]) {
    emitToolResult(sys, { name: NAVIGATE, ok: false, error: err });
    emitToolResult(sys, { name: NAVIGATE, ok: false, error: err });
  }
  emitLlmReturn(sys); // keep only the persistent ledger lines in view
  assert.equal(failureLines(await renderMsgs(resultsBlock(store))).length, 3, "three persistent lines");

  // Shed the single oldest failure entry (errA).
  assert.doesNotThrow(() => emitContextFull(sys, 1), "shedding a failure entry must not throw");
  let fails = failureLines(await renderMsgs(resultsBlock(store)));
  ...
  // Over-large round clamps to empty and never throws or goes negative.
...
test("F2 (f): ledger bounded — max+1 distinct (tool,error) failures keep only maxFailureNotices (oldest dropped)", async () => {
  ...
  // max+1 distinct errors, each at count 2 so each WOULD render if retained.
  for (let i = 0; i < max + 1; i++) {
    emitToolResult(sys, { name: NAVIGATE, ok: false, error: `err-${i}` });
    emitToolResult(sys, { name: NAVIGATE, ok: false, error: `err-${i}` });
  }
  emitLlmReturn(sys);
  const fails = failureLines(await renderMsgs(resultsBlock(store)));
...
for (const bad of [
  ...
  { label: "ok:false with no error field", value: { id: "x", at: 1, ok: false, name: NAVIGATE } },
]) {
  test(`F2 (g): malformed tool.result (${bad.label}) never throws through the failure path`, async () => {
    ...
    // A foreign failure must not create a ledger entry either.
    emitToolResult(sys, { name: "web-search.search", ok: false, error: "not mine" });
    emitToolResult(sys, { name: "web-search.search", ok: false, error: "not mine" });
    emitLlmReturn(sys);
    assert.deepEqual(
      failureLines(await renderMsgs(resultsBlock(store))),
...
test("F2 (h): a single (count-1) failure renders NO persistent line (only its normal fresh result that frame)", async () => {
  const { store, sys } = await setup({});
  emitToolResult(sys, { name: READ_PAGE, ok: false, error: "one-off" });

  // Before the return: the normal fresh error result renders (existing behavior),
  // but there is NO persistent 'failed Nx in a row' line at count 1.
  const before = await renderMsgs(resultsBlock(store));
  assert.equal(before.length, 1, "the normal fresh error result renders this frame");
  assert.match(String(before[0].content), /Error:\s*one-off/, "it is the normal fresh error entry");
  assert.deepEqual(failureLines(before), [], "no persistent line at count 1");

  // After the return: the fresh result is cleared and a count-1 failure leaves
  // NOTHING behind.
  emitLlmReturn(sys);
  assert.deepEqual(
    await renderMsgs(resultsBlock(store)),
